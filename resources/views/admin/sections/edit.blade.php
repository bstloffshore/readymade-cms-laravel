...
<div class="modal-header">
    <h3 class="modal-title" id="ajaxModalLabel"><b>Edit:: General Section</b></h3><small>({{ $section->section_title_en }})</small>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
    </button>
</div>
<div class="modal-body">
    <form method="POST" action="{{ route('sections.updateSection') }}" accept-charset="UTF-8" id="section_form" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="id" value="{{$section->id}}">
            ...
            <div class="row">
                <div class="col-sm-6 col-xs-12 form-group @error('section_title_en') has-error @enderror">
                    ...
                    value="{{$section->section_title_en}}">
                ...
                <div class="col-sm-6 col-xs-12 form-group @error('section_title_ar') has-error @enderror">
                    ...
                    value="{{$section->section_title_ar}}">
            ...
            <div class="row">
                <div class="col-sm-6 col-xs-12 form-group @error('icon') has-error @enderror">
                    ...
                    <input class="form-control" placeholder="Enter Section Title" name="icon" type="text" id="icon" value="{{$section->icon}}">
                    @error('icon')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-sm-6 col-xs-12 form-group @error('icon_file') has-error @enderror">
                    ...
                    <input class="form-control" type="hidden" name="edit_icon_file"  id="edit_icon_file" value="{{$section->icon_file}}">
                    <p><img src="{{ asset('public/images/section/thumb') }}/{{ $section->icon_file }}"></p>
                    @error('icon_file')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-sm-6 col-xs-12 form-group @error('status') has-error @enderror">
                    ...
                    <select id="status" name="status" class="form-control" >
                        <option value="1" @if($section->status==1) selected @endif>Yes</option>
                        <option value="0" @if($section->status==0) selected @endif>No</option>
                    </select>
                    @error('status')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-sm-6 col-xs-12 form-group @error('sort_order') has-error @enderror">
                    ...
                    <input class="form-control" placeholder="Enter Sort Order" name="sort_order" type="text" id="sort_order" value="{{$section->sort_order}}">
                    @error('sort_order')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-sm-12 col-xs-12 form-group @error('description_en') has-error @enderror">
                    ...
                    <textarea class="form-control" name="description_en" id="description_en" rows="3">{{$section->description_en}}</textarea>
                ...
                <div class="col-sm-12 col-xs-12 form-group @error('description_ar') has-error @enderror">
                    ...
                    <textarea class="form-control" name="description_ar" id="description_ar" rows="3">{{$section->description_ar}}</textarea>
  ...
  @else
  @include('admin.access-denied-for-modal')
  @endif
