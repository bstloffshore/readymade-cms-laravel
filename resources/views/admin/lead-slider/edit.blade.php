...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><small><b>{{ __('breadcrumb.edit_lead_slider') }}</b></small></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('breadcrumb.dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('breadcrumb.edit_lead_slider') }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-body">
    <form method="POST" action="{{ route('lead-sliders.update', ['lead_slider' => $slider->id]) }}" accept-charset="UTF-8" id="validate_form" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="row">
            <div class="col-sm-6 col-xs-12 form-group @error('image') has-error @enderror">
                ...
                <p><a href="{{ asset('public/storage/lead-slider/large') }}/{{ $slider->image }}" data-toggle="lightbox" data-title="{{ $slider->image_title }}">
                    <img width=50 height=50 src="{{ asset('public/storage/lead-slider/large') }}/{{ $slider->image }}" class="img-fluid mb-2" alt="{{ $slider->image_alt }}"/>
                    </a>
                    </p>
            ...
            <div class="col-sm-6 col-xs-12 form-group @error('image_title') has-error @enderror">
                ...
                <input class="form-control" name="image_title" type="text" id="image_title" placeholder="Enter {{ __('lead-slider.image_title') }}" value="{{ $slider->image_title }}">
                @error('image_title')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            <div class="col-sm-6 col-xs-12 form-group @error('image_alt') has-error @enderror">
                ...
                <input class="form-control" name="image_alt" type="text" id="image_alt" placeholder="Enter {{ __('lead-slider.image_alt') }}" value="{{ $slider->image_alt }}">
                @error('image_alt')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
